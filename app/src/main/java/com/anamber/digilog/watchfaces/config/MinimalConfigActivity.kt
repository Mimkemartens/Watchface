...
class MinimalConfigActivity : BaseConfigActivity() {


    override val watchFaceServiceClass: Class<*>
        get() = MinimalConfigData.watchFaceServiceClass

    override val sharedPrefFileKeyResId: Int
        get() = MinimalConfigData.PREFERENCE_FILE_KEY

    override fun getDataToPopulateAdapter(context: Context): ArrayList<BaseConfigData.ConfigItemType> {
        return MinimalConfigData.getDataToPopulateAdapter(context)
    }
