...
class PentagonWatchFaceService : BaseWatchFaceService() {

    override val sharedPrefFileKey: String
        get() = getString(PentagonConfigData.PREFERENCE_FILE_KEY)
    ...
    internal inner class CustomEngine : BaseWatchFaceService.Engine() {
        ...
        private val mWatchFaceDrawer = PentagonWatchFaceDrawer()

        override fun onSurfaceChanged(holder: SurfaceHolder, format: Int, width: Int, height: Int) {
            super.onSurfaceChanged(holder, format, width, height)

            mCompilationDistanceFromBorder = mCenterX * 0.6f
        ...
        override fun drawWatchFace(canvas: Canvas?, secondsRotation: Float, minutes: Int, hoursRotation: Float, showSecondHand: Boolean) {
            ...
            mComplicationDrawableSparseArray.get(BaseWatchFaceService.RIGHT_COMPLICATION_ID).bounds =
                    MathHelper.getComplicationRect(mCenterX, mCenterY, mCompilationDistanceFromBorder, mComplicationSize, hoursRotation + 120)

            mComplicationDrawableSparseArray.get(BaseWatchFaceService.LEFT_COMPLICATION_ID).bounds =
                    MathHelper.getComplicationRect(mCenterX, mCenterY, mCompilationDistanceFromBorder, mComplicationSize, hoursRotation + 240)
