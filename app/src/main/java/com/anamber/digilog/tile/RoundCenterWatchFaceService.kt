...
import com.anamber.digilog.presentation.RoundCenterConfigData
import com.anamber.digilog.tile.MathHelper.getComplicationRect

class RoundCenterWatchFaceService : BaseWatchFaceService() {

    internal override val sharedPrefFileKey: String
        get() = getString(RoundCenterConfigData.PREFERENCE_FILE_KEY)
    ...
    internal inner class CustomEngine : BaseWatchFaceService.Engine() {
        ...
        private val mWatchFaceDrawer = RoundCenterWatchFaceDrawer()


        override fun onSurfaceChanged(holder: SurfaceHolder, format: Int, width: Int, height: Int) {
            super.onSurfaceChanged(holder, format, width, height)

            mCompilationDistanceFromBorder = mWidth / 6.5f
            mWatchFaceDrawer.calculateSizes(width, height, mCenterX, mCenterY)
        }

        internal override fun drawWatchFace(canvas: Canvas?, secondsRotation: Float, minutes: Int, hoursRotation: Float, showSecondHand: Boolean) {
